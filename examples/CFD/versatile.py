...
class Cylinder(BGKSim):
    ...
    def set_boundary_conditions(self):
        ...
        # square = np.abs(xx - cx) + np.abs(yy-cy) <= (diam/2.)
        # square = coord[square]
        ...
        dom = np.loadtxt("./examples/CFD/versatile_domain/domain.txt")
        obstacle = np.array([[0,0]])
        for i in range(self.nx):
            for j in range(self.ny):
                if np.abs(dom[i,j]-1.0)<0.0001:
                    obstacle = np.concatenate([obstacle,np.array([[i,j]])],axis=0)
        obstacle = obstacle[1:]
        print(obstacle.dtype)
        self.BCs.append(BounceBack(tuple(obstacle.T), self.gridInfo, self.precisionPolicy))
        # Outflow BC
    ...
    def output_data(self, **kwargs):
        ...
        if timestep > 0.5 * niter_max:
            ...
            save_image(timestep, u, prefix='./output/') # prefix: specify directory to store output result 
